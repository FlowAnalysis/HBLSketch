#include <stdio.h>
#include <stdlib.h>
#include <unordered_map>
#include <vector>
#include <algorithm>
//#include <iostream>

#include "../elastic/ElasticSketch.h"
using namespace std;

#define START_FILE_NO 1
#define END_FILE_NO 3


struct FIVE_TUPLE{	char key[13];	};
typedef vector<FIVE_TUPLE> TRACE;
TRACE traces[END_FILE_NO - START_FILE_NO + 1];

void ReadInTraces(const char *trace_prefix)
{
	for(int datafileCnt = START_FILE_NO; datafileCnt <= END_FILE_NO; ++datafileCnt)
	{
		char datafileName[100];
		sprintf(datafileName, "%s%d.dat", trace_prefix, datafileCnt - 1);
		FILE *fin = fopen(datafileName, "rb");

		FIVE_TUPLE tmp_five_tuple;
		traces[datafileCnt - 1].clear();
		while(fread(&tmp_five_tuple, 1, 13, fin) == 13)
		{
			traces[datafileCnt - 1].push_back(tmp_five_tuple);
		}
		fclose(fin);

		printf("Successfully read in %s, %ld packets\n", datafileName, traces[datafileCnt - 1].size());
		//printf("Successfully read %s\n", traces[datafileCnt - 1]);
		//cout << traces[datafileCnt - 1][1].key;
	}
	printf("\n");
}

int main()
{
	ReadInTraces("../../../data/");


#define HEAVY_MEM (300 * 1024)
#define BUCKET_NUM (HEAVY_MEM / 64)
#define BUFFER_MEM (20 * 1024)
#define BBUCKET_NUM (BUFFER_MEM / 8)
#define TOT_MEM_IN_BYTES (600 * 1024)
	ElasticSketch<BUCKET_NUM, TOT_MEM_IN_BYTES, BBUCKET_NUM> *elastic = NULL;
	//printf("bbucket_num: %d\n", BUCKET_NUM);
    //printf("bTOT_MEM_IN_BYTES: %d\n", TOT_MEM_IN_BYTES);



	for(int datafileCnt = START_FILE_NO; datafileCnt <= END_FILE_NO; ++datafileCnt)
	{
		unordered_map<string, int> Real_Freq;
		elastic = new ElasticSketch<BUCKET_NUM, TOT_MEM_IN_BYTES, BBUCKET_NUM>();

		int packet_cnt = (int)traces[datafileCnt - 1].size();
		for(int i = 0; i < packet_cnt; ++i)
		{
			//cout << *(uint8_t*)(traces[datafileCnt - 1][i].key);
			elastic->insert((uint8_t*)(traces[datafileCnt - 1][i].key));
			// elastic->quick_insert((uint8_t*)(traces[datafileCnt - 1][i].key));

			string str((const char*)(traces[datafileCnt - 1][i].key), 4);
			Real_Freq[str]++;
		}

		double ARE = 0;
		int a=0;
		int b=0;
		int c=0;
		int d=0;
		int g=0;
		int h=0;
		uint32_t e[100000];
		int f[100000];
		//int fsr=(int)Real_Freq.size();
		//int fsize[fsr];
		int threshold=packet_cnt * 1 / 1000;
		for(unordered_map<string, int>::iterator it = Real_Freq.begin(); it != Real_Freq.end(); ++it)
		{
			uint8_t key[4];
			memcpy(key, (it->first).c_str(), 4);
			int est_val = elastic->query(key);
			a=a+est_val;
			b=b+it->second;
			int dist = std::abs(it->second - est_val);
			ARE += dist * 1.0 / (it->second);


			//printf("real heavy hitters <srcIP, count>\n");

			if (it->second >= threshold)
			{
				uint32_t s;
				memcpy(&s, it->first.c_str(), 4);
				//printf("<%x, %d>", s, it->second);
				e[c]=s;
				f[c]=it->second;
				c=c+1;
				/*if(++d % 5 == 0)
				{
					printf("\n");
				}
				else printf("\t");*/
			}
		}
		ARE /= (int)Real_Freq.size();
		
		vector<double> dist;
		//elastic->get_distribution(dist);

#define HEAVY_HITTER_THRESHOLD(total_packet) (total_packet * 1 / 1000)
//#define HEAVY_HITTER_THRESHOLD(thershold) (fsize[thershold])
		printf("threshold=%d\n",threshold);
		vector< pair<string, int> > heavy_hitters;
		elastic->get_heavy_hitters(HEAVY_HITTER_THRESHOLD(packet_cnt), heavy_hitters);
		//elastic->get_heavy_hitters(HEAVY_HITTER_THRESHOLD(thershold), heavy_hitters);

		printf("%d.dat: ARE=%.6lf\t\n", datafileCnt - 1, ARE);
		//printf("est_cardinality=%d\t", elastic->get_cardinality());
		//printf("entropy=%.3lf\n", elastic->get_entropy());
		/*printf("flow size distribution: <flow size, count>\n");
		for(int i = 0, j = 0; i < (int)dist.size(); ++i)
			if(dist[i] > 0)
			{
				printf("<%d, %d>", i, (int)dist[i]);
				if(++j % 10 == 0)
					printf("\n");
				else printf("\t");
			}*/
		//printf("\n");
				
		printf("real flows num: %d\n", (int)Real_Freq.size());
		printf("<est pac num,real pac num>,<%d, %d>\n", a, b);

		printf("heavy hitters: <srcIP, count>, threshold=%d\n", HEAVY_HITTER_THRESHOLD(packet_cnt));
		
		printf("real heavy hitters num: %d\n", c);

		printf("heavy hitters num: %d\n", (int)heavy_hitters.size());
		for(int i = 0, j = 0; i < (int)heavy_hitters.size(); ++i)
		{
			uint32_t srcIP;
			memcpy(&srcIP, heavy_hitters[i].first.c_str(), 4);
			/*printf("<%.8x, %d>", srcIP, heavy_hitters[i].second);
			//printf("%x\n",heavy_hitters[i].first.c_str());
			if(++j % 5 == 0)
				printf("\n");
			else printf("\t");*/

			//compare heavy hitters
			for(int z=0; z<c; ++z)
			//for(int z=0; z<(int)heavy_hitters.size(); ++z)
			{
				if(srcIP==e[z])
				{
					if(heavy_hitters[i].second==f[z])
					{
						g=g+1;
					}
					else
					{
						h=h+1;
						//printf("mismatching<%.8x>\n", srcIP);
					}
				}
			}

		}
		//printf("\n");
		printf("equal:%d,not equal:%d", g, h);
		printf("\n");
		printf("\n");

		delete elastic;
		Real_Freq.clear();
	}
}	