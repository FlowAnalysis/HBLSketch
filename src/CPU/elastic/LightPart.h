...
/* insertion */
	void insert(uint8_t *key, int f = 1)
	{
        ...
        double b=hash_val * a;
        //printf("b=%lf\n",b);
        double c=b-uint32_t(b);
        //printf("c=%lf\n",c);
        uint32_t pos = (uint32_t)(c * (uint32_t)counter_num);*/
...
/* compress */
    void compress(int ratio, uint8_t *dst) 
    {
        ...
        printf("compress,compress,compress\n");
    }
	...
	int query_compressed_part(uint8_t *key, uint8_t *compress_part, int compress_counter_num) 
	{
        ...
        printf("query_compressed_part,query_compressed_part\n");
...
/* other measurement task */
    int get_compress_width(int ratio) { printf("get_compress_width,get_compress_width\n");return (counter_num / ratio); }
    int get_compress_memory(int ratio) {	printf("get_compress_memory,get_compress_memory\n");return (uint32_t)(counter_num / ratio); }
