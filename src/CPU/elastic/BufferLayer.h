#ifndef _BUFFERLAYER_H_
#define _BUFFERLAYER_H_
...
template<int Bbucket_num>
class BufferLayer
{
public:
	alignas(64) BBucket Bbuckets[Bbucket_num];
	...
	void clear()
	{
		memset(Bbuckets, 0, sizeof(BBucket) * Bbucket_num);
	}


/* insertion */
	int insert(uint8_t *key, uint8_t *Bswap_key, uint32_t &Bswap_val, uint32_t f = 1)
	{
		uint32_t fp;
		int pos = CalculateFP(key, fp);	
		//printf("in buffer layer insert!\n");


		/*const __m256i item = _mm256_set1_epi32((int)fp);
		__m256i *keys_p = (__m256i *)(&Bbuckets[pos].key);
		int matched = 0;

		__m256i a_comp = _mm256_cmpeq_epi32(item, keys_p[0]);
		matched = _mm256_movemask_ps((__m256)a_comp);*/

		if (Bbuckets[pos].key == fp)
		{
		...
		{
			*((uint32_t*)Bswap_key) = Bbuckets[pos].key;
			Bswap_val = Bbuckets[pos].val;
			Bbuckets[pos].key = fp;
			Bbuckets[pos].val = f;
			return 1;
		}
...
/* interface */
	...
	{
		return Bbucket_num * sizeof(BBucket);
	}
	int get_bucket_num()
	{
		return Bbucket_num;
...
private:
	...
	{
		fp = *((uint32_t*)key);
		return CalculateBucketPos(fp) % Bbucket_num;
	}
};
